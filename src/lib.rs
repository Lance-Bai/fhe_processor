...
mod circuit_bootstrapping_tests {
    ...
    use crate::{
        operations::{
            lut::single_operator_calculate, lut_manager::ArithmeticLookupManager,
            manager::concat_ggsw_lists, operand::ArithmeticOp, operation::Operation,
        },
        processors::{
            cbs_4_bits::{
                circuit_bootstrapping_4_bits_at_once, circuit_bootstrapping_4_bits_at_once_rev_tr,
            },
            key_gen::allocate_and_generate_new_reused_lwe_key,
            lwe_stored_ksk::allocate_and_generate_new_stored_reused_lwe_keyswitch_key,
        },
        utils::instance::{SetI, ZeroNoiseTest},
    };
    ...
    //     manager.add_operations(&[ArithmeticOp::Addi, ArithmeticOp::Subi, ArithmeticOp::Mulli]);
    ...
    //         let cbs_start = Instant::now();
    //         // circuit_bootstrapping_4_bits_at_once_(
    //         //     &lwe_low,
    //         //     &mut fourier_gsw_list_low,
    //         //     fourier_bsk,
    //         //     &auto_keys,
    ...
    //         let total_cbs_time = cbs_times.iter().map(|d| d.as_secs_f64()).sum::<f64>();
    //         let avg_cbs_time = total_cbs_time / cbs_times.len() as f64;
    //         println!("Average LWE cbs time: {:.3} s", avg_cbs_time);

    //         let total_lut_time = lut_times.iter().map(|d| d.as_secs_f64()).sum::<f64>();
    //         let avg_lut_time = total_lut_time / lut_times.len() as f64;
    //         println!("Average LUT calculation time: {:.3} s", avg_lut_time);
    //     }
    // }
}
