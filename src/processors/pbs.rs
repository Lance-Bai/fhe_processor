...
use tfhe::core_crypto::{
    ...
    fft_impl::fft64::{crypto::bootstrap::FourierLweBootstrapKeyView, math::fft::FftView},
    prelude::*,
};

pub fn pbs_many_lut_after_ms_before_extract_lead_one<Scalar, OutputCont>(
    body: &MonomialDegree,
    ...
    OutputCont: ContainerMut<Element = Scalar>,
{
    ...
    for (acc_idx, mut glev_chunk) in glev_out.chunks_mut(lut_count).enumerate() {
        ...
        let mut accumulator = (0..polynomial_size.0)
            .map(|i| {
                let k = i % lut_count;
                let j = acc_idx * lut_count + k;

                let log_scale = Scalar::BITS - 1 - j * cbs_base_log.0;
                // println!("i = {}, j = {}, log_scale = {}", i, j, log_scale);
